...
for i in range(1):
    area_ratios_at_step = []
    transform_matrices = beam.compute_joint_group_optimisation(6, 1.0)
    for joint_group in beam.get_joints_by_group():
        print(f"Joint group degree of freedom: {joint_group.get_degree_of_freedom()}")
        for joint in joint_group.get_joints():
            for face in joint.get_faces():
                projected_points = face.project_points_onto_face(beam_point_cloud)
                for pt in projected_points:
                    pts.append(Rhino.Geometry.Point3d(pt[0], pt[1], pt[2]))
...
# colors = ["red", "salmon", "blue", "navy", "gold", "orange", "tan", "green", "lime", "aquamarine"]
# labels = ["face 1, joint 1", "face 2, joint 1", "face 1, joint 2", "face 2, joint 2", "face 1, joint 3", "face 2, joint 3", "face 3, joint 3", "face 1, joint 4", "face 2, joint 4", "face 3, joint 4"]
# plt.figure(figsize=(10, 6))
# for i in range(len(area_ratios[0])):
#     plt.plot(range(10), [area_ratios[j][i] for j in range(10)], label=labels[i], color=colors[i])
# plt.axhline(y=1, color='black', linestyle='--', linewidth=1, label='y=1')
# plt.xlabel('Step')
# plt.ylabel('Area Ratio')
# plt.title('Area Ratios Over Steps')
# plt.legend()
# plt.show()